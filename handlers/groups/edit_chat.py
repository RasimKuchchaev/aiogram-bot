...
# command /set_photo или !set_photo
@dp.message_handler(IsGroup(), Command("set_photo", prefixes="!/"), AdminFilter())
async def set_new_photo(message:types.Message):
    source_message = message.reply_to_message
    photo = source_message.photo[-1]      # размеры фото [-1] самое последние самое большое разрешение
    photo = await photo.download(destination=io.BytesIO())    # фото храню в памяти в байтах
